...
- 과제 : [필수과제2] RNN-based Language Model
...
## 2. 새로 알게된 내용 / 고민한 내용 (강의, 과제, 퀴즈)
- **실습 코드 : 3_basic_rnn**
    - pytorch.nn.RNN의 Input은 `batch_first = False`가 default이고 이 때 ($L, N, H_{in}$)으로 입력받는다. ($L$ : sequence length, $N$ : batch size)
    - pack_padded_sequence를 이용해 불필요한 연산을 제거할 수 있다. 
    - packing한 것을 풀어줄 때에는 pad_packed_sequence을 이용한다.
- **실습 코드 : 4_fancy_rnn**
    - LSTM, GRU, Bidirectional의 동작 원리에 대해 실습할 수 있었다.
- **필수과제3 : RNN-based Language Model**
...
- 과제 : [필수과제3] Subword-level Language Model
...
- 과제 : [필수과제4] Preprocessing for NMT Model
...
- **BLEU(BiLingual Evaluation Understudy) score**
    ...
    - brevity penality라는 항을 추가하여 recall을 역할을 대신한다.
    - n-gram을 통해 예측한 값의 기하평균을 사용한다.
...
- 과제 : [선택과제1] BERT Fine-tuning with transformers
